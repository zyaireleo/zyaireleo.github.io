...
- 线程私有内存区域：java虚拟机栈、程序技术器、本地方法栈。生命周期和线程的生命周期一致。

  ![](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E8%BF%90%E8%A1%8C%E6%97%B6%E6%95%B0%E6%8D%AE%E5%8C%BA.jpg)
...
- **是什么？**
  ...
  * 查看.class文件行号
    ...
    **创建User类**
    ...
    ![](java%E8%99%9A%E6%8B%9F%E6%9C%BA/User%E7%B1%BB.png)

    javac User.java编译

    javap -l User.class查看如下

    ![](java%E8%99%9A%E6%8B%9F%E6%9C%BA/class%E6%96%87%E4%BB%B6%E8%A1%8C%E5%8F%B7.png)
...
public class A {
    ...
    //局部变量表: 可以理解为方法里面的变量
    //操作数栈: 例如 c=a*b这种
    //动态链接: 对对象的引用, 例如 User user = new user()的这种
    //方法出口: 就是return语句
...
- 是什么？
  ...
  - 用于作用域本地方法执行的一块Java内存区域，和虚拟机栈不同的是这里执行的是native方法，而本地虚拟机栈执行的是java方法，会被编译成class文件。

- 什么是本地（native）方法？

  - 是一个修饰符，不能有方法体。调用java之外的方法。

- 这又是什么呢？

  - 与Java虚拟机栈相同，每个方法在执行的同时都会创建一个栈帧（Stack Framel）用于存储局部变量表、操作数栈、动态链接、方法出口等信息。每一个方法从调用直至执行完成的过程，就对应着一个栈帧在虚拟机栈中入栈到出栈的过程

- Hotshot（**jvm其中的一种实现方式**）将Java虚拟机栈和本地方法栈合二为一
...
![](java%E8%99%9A%E6%8B%9F%E6%9C%BA/user%E5%AD%97%E8%8A%82%E7%A0%81%E6%96%87%E4%BB%B6)
...
- 拿来干什么？
  ...
  - Hotspot使用永久代（对象经常被使用，经过多次垃圾回收后还没有被回收掉，就把它放在永久代里面）来实现方法区   JRockit、IBM J9VM Java堆一样管理这部分内存
...
```java
public class A {
	//在方法区会有维护StringTable的表(本质上是一个HashSet的结构)来存储常量
    public static void main(String[] args) {
        String a = "abc";
        String b = "abc";
        ...
        //intern (native方法) 用来将java堆里面的内存移到java方法区中, 下面结果为true
...
![Java对象创建底层步骤](java%E8%99%9A%E6%8B%9F%E6%9C%BA/Java%E5%AF%B9%E8%B1%A1%E5%88%9B%E5%BB%BA%E5%BA%95%E5%B1%82%E6%AD%A5%E9%AA%A4.jpg)

- **对象创建的流程步骤包括哪些：**

  1. 虚拟机遇到一条new指令时，首先检查这个对应的类能否在常量池中定位到一个类的符号引用 (如果能够定位到, 就说明这个对象在常量池存在)

  2. 判断这个类是否已被加载、解析和初始化

  3. 为这个新生对象在Java堆中分配内存空间，其中Java堆分配内存空间的方式主要有以下两种

    ![对象分配空间方式](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E5%AF%B9%E8%B1%A1%E5%88%86%E9%85%8D%E7%A9%BA%E9%97%B4%E6%96%B9%E5%BC%8F.jpg)

    - **指针碰撞 **(就是将使用内存进行扩大,  空闲内存减小,  新增的内存就是为对象分配的内存)
      - 分配内存空间包括开辟一块内存和移动指针两个步骤
      - 非原子步骤可能出现并发问题，Java虚拟机采用CAS配上失败重试的方式保证更新操作的原子性
    - **空闲列表** ( 列表存储并维护堆内存中空闲内存的地址 )
...
- 代码分析对象执行的过程
  ...
  加载类(例如执行static方法)--->构造方法--->普通方法

  

##### 不可不知的对象结构

![对象内存布局](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E5%AF%B9%E8%B1%A1%E5%86%85%E5%AD%98%E5%B8%83%E5%B1%80.jpg)

1. 对象头用于存储对象的元数据信息：
  - GC分代年龄: 不同的分代年龄,  GC回收时会采用不同的回收算法(总共三种)
  - 锁状态: 为线程安全锁设置的一个概念
...
2. 实例数据存储的是真正有效数据，如各种字段内容，各字段的分配策略为longs/doubles、ints、shorts/chars、bytes/boolean、oops(ordinary object pointers)，相同宽度的字段总是被分配到一起，便于之后取数据。父类定义的变量会出现在子类定义的变量的前面。
3. 对齐填充部分仅仅起到占位符的作用 (为了保证对象头字节数为8的倍数 )
...
- 当我们在堆上创建一个对象实例后，就要通过虚拟机栈中的reference类型数据来操作堆上的对象。现在主流的访问方式有两种（HotSpot虚拟机采用的是第二种）：
  ...
  - 类型数据和实例数据:
    ...
    类型数据: 	用来描述class文件有哪些数据类型,  实现了哪些接口等等的数据

  - **使用句柄访问对象**。即reference中存储的是对象句柄的地址，而句柄中包含了对象实例数据与类型数据的具体地址信息，相当于二级指针。

    ![对象句柄访问](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E5%AF%B9%E8%B1%A1%E5%8F%A5%E6%9F%84%E8%AE%BF%E9%97%AE.jpg)

  - **直接指针访问对象**。即reference中存储的就是对象地址，相当于一级指针**。**

    ![对象访问定位直接访问](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E5%AF%B9%E8%B1%A1%E8%AE%BF%E9%97%AE%E5%AE%9A%E4%BD%8D%E7%9B%B4%E6%8E%A5%E8%AE%BF%E9%97%AE.jpg)
...
![引用计数法](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E5%BC%95%E7%94%A8%E8%AE%A1%E6%95%B0%E6%B3%95.jpg)

- 引用计数法存在的特点分析

- 优缺点

  - 最大的特点就是简单，通过对象的引用数就可以判断是否回收
...
- 配置启动参数打印GC日志
  ...
  public class ReferenceCountingGC {
      ...
      public static void main(String[] args) {
          ...
          //相当于切断了虚拟机栈对堆的引用, 这样可以查看GC到底有没有使用引用计数器来判断是否回收(引用计数法有循环引用的问题)
          objA = null;
          objB = null;
...
![可达性分析法](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E5%8F%AF%E8%BE%BE%E6%80%A7%E5%88%86%E6%9E%90%E6%B3%95.jpg)
...
![标记清除算法](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E6%A0%87%E8%AE%B0%E6%B8%85%E9%99%A4%E7%AE%97%E6%B3%95.jpg)
...
![复制算法](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E5%A4%8D%E5%88%B6%E7%AE%97%E6%B3%95.jpg)
...
- 使用场景
  - 现在的商业虚拟机都采用这种收集算法来回收**新生代**，研究表明，新生代中的对象 98%是“朝生夕死”的，所以并不需要按照 1:1 的比例来划分内存空间，而是将内存分为一块较大的 Eden 空间和两块较小的 Survivor 空间，每次使用 Eden 和其中一块 Survivor。  Survivor from  和Survivor to  ，内存比例 8：1：1
- 回收算法
  - 当回收时，将 Eden 和 Survivor 中还存活着的对象一次性地复制到另外一块 Survivor  ( Survivor to ,  回收完成后会把所有存活的对象拷贝到Survivor from 里面 ) 空间上，最后清理掉 Eden 和刚才用过的 Survivor 空间。HotSpot 虚拟机默认 Eden 和 Survivor 的大小比例是 8:1, 也就是每次新生代中可用内存空间为整个新生代容量的 90% (80%+10%），只有 10% 的内存会被“浪费”。当然，98%的对象可回收只是一般场景下的数据，我们没有办法保证每次回收都只有不多于 10%的对象存活，当 Survivor 空间不够用时，需要依赖其他内存（这里指老年代）进行分配担保（Handle Promotion）。



##### 标记整理算法与分代收集算法

![标记整理](java%E8%99%9A%E6%8B%9F%E6%9C%BA/%E6%A0%87%E8%AE%B0%E6%95%B4%E7%90%86.jpg)
...
![Serial垃圾收集器](java%E8%99%9A%E6%8B%9F%E6%9C%BA/Serial%E5%9E%83%E5%9C%BE%E6%94%B6%E9%9B%86%E5%99%A8.jpg)
...
![ParNew垃圾收集器](java%E8%99%9A%E6%8B%9F%E6%9C%BA/ParNew%E5%9E%83%E5%9C%BE%E6%94%B6%E9%9B%86%E5%99%A8.jpg)
...
![CMS垃圾收集器](java%E8%99%9A%E6%8B%9F%E6%9C%BA/CMS%E5%9E%83%E5%9C%BE%E6%94%B6%E9%9B%86%E5%99%A8.jpg)
...
- 步骤流程：
  ...
  * 并发标记（CMS concurrent mark --------并发标记阶段就是进行 GC RootsTracing 的过程（可以是多线程）
...
![G1](java%E8%99%9A%E6%8B%9F%E6%9C%BA/G1.jpg)
...
- 特点：
  ...
  - G1 中每个 Region 都有一个与之对应的 Remembered Set（用来记录和维护Reference关系），当进行内存回收时，在 GC 根节点的枚举范围中加入 Remembered Set 即可保证不对全堆扫描也不会有遗漏    检查Reference引用的对象是否处于不同的Region
...
- **G1的优势**
  ...
  - 可预测的停顿：（**因为他维护了region和RememberSet，可以预估停顿时间**）这是 G1 相对于 CMS 的另一大优势，降低停顿时间是 G1 和 CMS 共同的关注点，但 G1 除了追求低停顿外，还能建立可预测的停顿时间模型
